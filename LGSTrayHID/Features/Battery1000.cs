...
namespace LGSTrayHID.Features
{
    public static class Battery1000
    {
        ...
        {
            var buffer = new byte[7] { 0x10, device.DeviceIdx, device.FeatureMap[0x1000], 0x00 | HidppDevices.SW_ID, 0x00, 0x00, 0x00 };
            var ret = await device.Parent.WriteRead20(device.Parent.DevShort, buffer);
            ...
            return new BatteryUpdateReturn((int)batPercent, status, mv);
        }
    }
} 